package com.zhangpengfei.spark.udf
...
object SimpleDemo {
  def main(args: Array[String]): Unit = {
    ...
    //使用函数
    val df2 = df.withColumn("addr-ex", callUDF(splitAddrFunc.toString(), $"addr", df("addr")))
    ...
    println("打印SQL语句执行结果")
    sqlContext.sql("select name,len(name) from user where longLength(name,2)").foreach(println(_))

    println("打印数据过滤结果")
    df.filter("longLength(name,2)").foreach(println(_))
